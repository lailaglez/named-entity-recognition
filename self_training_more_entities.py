...
class SelfTrain:
    ...
    def step(self):
        ...
        best_predictions = self.find_best_entity_predictions(predictions, limit)
        indexes = np.array([b[1] for b in best_predictions])
        ...
        for p in best_predictions:
            ...
            assert len(prediction) == len(self.structured_y[real_index]) and len(prediction) == self.lengths[
                real_index]

            self.structured_y[real_index] = prediction
            for j in range(len(prediction)):
                self.classic_y[self.beginnings[real_index] + j] = prediction[j]

        if len(np.where(self.tweets_for_testing)[0]) == 0:
            self.ended = True

    def find_best_entity_predictions(self, predictions, limit):
        best_entity_predictions = []
        for i in range(len(predictions[0])):
            tweet = predictions[:, i]
            best_entity_predictions.append((best_prediction(tweet), i))

        best_entity_predictions.sort(key=lambda x: x[0].count('O')/len(x[0]))

        return best_entity_predictions[:limit]
...
for i in range(1):
    ...
    trainer = SelfTrain(n_init=1000, n_step=500,
                        structured_classifiers=[
...
# print(cnf_matrix)
# print(trainer.structured_classifiers)
# print(trainer.classic_classifiers)
